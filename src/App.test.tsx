...
vi.mock('./components/grimoireEditorFormatting', () => ({
  GrimoireFormattingToolbar: () => null,
  GrimoireFormattingToolbarController: () => null,
}))
...
describe('App', () => {
  ...
  it('opens a note window by loading only the requested entry', async () => {
    ...
    await waitFor(() => expect(window.__grimoireTest?.activeTabPath).toBe('/vault/project/test.md'))
  ...
  it('shows visible feedback when a menu-driven update check finds no eligible update', async () => {
    ...
    act(() => {
      window.__grimoireTest?.dispatchBrowserMenuCommand?.('app-check-for-updates')
    })
  ...
  it('shows the external AI setup dialog from the menu when AI onboarding is active', async () => {
    ...
    act(() => {
      window.__grimoireTest?.dispatchBrowserMenuCommand?.('vault-install-mcp')
    })
  ...
  it('defaults to All Notes when explicit organization is disabled in vault config', async () => {
    ...
    localStorage.setItem(`grimoire:vault-config:${workVaultPath}`, disabledWorkflowConfig)
