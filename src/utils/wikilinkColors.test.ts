...
const noteEntry = makeEntry({ path: '/vault/note/welcome-to-grimoire.md', filename: 'welcome-to-grimoire.md', title: 'Welcome to Grimoire', isA: 'Note' })
...
describe('findEntryByTarget', () => {
  ...
  it('matches by relative path (folder/slug)', () => {
    expect(findEntryByTarget(allEntries, 'note/welcome-to-grimoire')).toBe(noteEntry)
  })

  it('matches by relative path with pipe syntax', () => {
    expect(findEntryByTarget(allEntries, 'note/welcome-to-grimoire|Welcome!')).toBe(noteEntry)
  })
...
describe('resolveWikilinkColor', () => {
  ...
  it('resolves relative-path wikilink target to correct type color', () => {
    const result = resolveWikilinkColor(allEntries, 'note/welcome-to-grimoire')
    expect(result.isBroken).toBe(false)
