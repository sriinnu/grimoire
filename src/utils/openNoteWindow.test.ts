...
describe('openNoteWindow', () => {
  ...
  it('builds a root-app route that preserves the note window params', () => {
    const url = buildNoteWindowUrl('/vault/Folder/My Note.md', '/Users/srinivas/Grimoire Vault', 'AI / ML')
    const parsed = new URL(url, 'https://grimoire.localhost')

    expect(parsed.pathname).toBe('/')
    expect(parsed.searchParams.get('window')).toBe('note')
    expect(parsed.searchParams.get('path')).toBe('/vault/Folder/My Note.md')
    expect(parsed.searchParams.get('vault')).toBe('/Users/srinivas/Grimoire Vault')
  ...
  it('opens a new Tauri window with the encoded note route', async () => {
    ...
    await openNoteInNewWindow('/vault/Folder/My Note.md', '/Users/srinivas/Grimoire Vault', 'AI / ML')
    ...
    expect(JSON.parse(localStorage.getItem('grimoire:note-window:note-1776182400000') ?? '{}')).toEqual({
      notePath: '/vault/Folder/My Note.md',
      vaultPath: '/Users/srinivas/Grimoire Vault',
      noteTitle: 'AI / ML',
    })
  })
