...
describe('evaluateView', () => {
  ...
  it('filters by relationship contains wikilink', () => {
    const view: ViewDefinition = {
      ...
      filters: { all: [{ field: 'Related to', op: 'contains', value: '[[grimoire-app]]' }] },
    }
    const entries = [
      makeEntry({ title: 'Match', relationships: { 'Related to': ['[[grimoire-app|Grimoire App]]', '[[other]]'] } }),
  ...
  it('filters by property field', () => {
    const view: ViewDefinition = {
      ...
      filters: { all: [{ field: 'Owner', op: 'equals', value: 'Sriinu' }] },
    }
    const entries = [
      makeEntry({ title: 'Match', properties: { Owner: 'Sriinu' } }),
      makeEntry({ title: 'Other', properties: { Owner: 'Karthik' } }),
