...
describe('extractH1TitleFromContent', () => {
  ...
  it('preserves plain square brackets in the H1', () => {
    const content = '# [26Q2] Grimoire MVP'
    expect(extractH1TitleFromContent(content)).toBe('[26Q2] Grimoire MVP')
  })
...
describe('deriveDisplayTitleState', () => {
  ...
  it('keeps plain square brackets when deriving the display title from H1', () => {
    const content = '# [26Q2] Grimoire MVP\n\nBody'
    expect(deriveDisplayTitleState({ content, filename: 'grimoire-mvp.md' })).toEqual({
      title: '[26Q2] Grimoire MVP',
      hasH1: true,
    })
  })
})
