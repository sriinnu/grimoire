const RELEASE_HISTORY_URL = 'https://sriinnu.github.io/grimoire/'
...
const PLATFORM_METADATA: Record<StablePlatformKey, { buttonLabel: string; label: string }> = {
  ...
  'linux-x86_64': {
    buttonLabel: 'Download Grimoire for Linux',
    label: 'Linux',
  },
  'windows-x86_64': {
...
function buildStableDownloadPageContent(
  ...
  if (Object.keys(downloads).length > 0) {
    return {
      ...
      message: 'Preparing the latest stable Grimoire download for your platform.',
      shouldRedirect: true,
      title: 'Grimoire Stable Download',
    }
  }

  return {
    helperText: 'Use the button below to check the latest release history.',
    message: 'No stable Grimoire downloads are available yet.',
    shouldRedirect: false,
    title: 'Grimoire Stable Download Unavailable',
  }
}
      ...
      if (resolvedTarget) {
        ...
        if (message) {
          message.textContent = requiresMacChoice
            ...
            : 'Redirecting to the latest stable Grimoire download for ' + resolvedTarget.label + '.';
        }
