...
describe('useZoom', () => {
  ...
  it('dispatches grimoire-zoom-change event on zoomIn', () => {
    const handler = vi.fn()
    window.addEventListener('grimoire-zoom-change', handler)
    const { result } = renderHook(() => useZoom())
    handler.mockClear() // clear any init-phase dispatches
    act(() => result.current.zoomIn())
    expect(handler).toHaveBeenCalled()
    window.removeEventListener('grimoire-zoom-change', handler)
  })

  it('dispatches grimoire-zoom-change event on zoomOut', () => {
    const handler = vi.fn()
    window.addEventListener('grimoire-zoom-change', handler)
    const { result } = renderHook(() => useZoom())
    handler.mockClear()
  ...
  it('dispatches grimoire-zoom-change event on zoomReset', () => {
    resetVaultConfigStore()
    bindVaultConfigStore({ ...DEFAULT_VC, zoom: 1.2 }, vi.fn())
    const handler = vi.fn()
    window.addEventListener('grimoire-zoom-change', handler)
