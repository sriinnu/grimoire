...
describe('buildContextCapsulePackagePreview', () => {
  it('creates a review-only Markdown package from visible capsule context', () => {
    ...
    expect(pack.markdown).toContain('## Capsule Manifest')
    expect(pack.markdown).toContain('- Review mode: review-before-handoff')
    expect(pack.markdown).toContain('- Source-safe notes: 2')
    expect(pack.markdown).toContain('- Held local items: 2')
    expect(pack.markdown).toContain('- Trimmed graph items: 0')
    expect(pack.markdown).toContain('- Next gate: User-reviewed package only')
  ...
  it('keeps protected active capsule packages free of protected labels', () => {
    ...
    expect(pack.markdown).toContain(`Review receipt: ${pack.reviewReceipt}`)
    expect(pack.markdown).toContain('- Review mode: blocked-local')
    expect(pack.markdown).toContain('- Next gate: No agent handoff')
