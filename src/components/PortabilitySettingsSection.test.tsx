...
describe('PortabilitySettingsSection', () => {
  it('surfaces import, export, storage, and second-brain groups', () => {
    ...
    expect(screen.getByText('Preview Notion folder')).toBeInTheDocument()
    ...
    expect(screen.getByText('Export static HTML')).toBeInTheDocument()
    expect(screen.getByText('Preview JSON snapshot')).toBeInTheDocument()
  ...
  it('only shows the active action busy label while locking all buttons', () => {
    ...
    expect(screen.getByTestId('settings-export-markdown-zip')).toHaveTextContent('Export Markdown ZIP')
