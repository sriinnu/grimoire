...
function actionCopy(status: McpStatus) {
  if (isConnected(status)) {
    return {
      description: 'Grimoire is already connected to external AI tools for this vault. Reconnect to refresh the configuration, or disconnect to remove Grimoire from those third-party config files.',
  ...
  return {
    description: 'Grimoire can add its MCP server to external AI tools for this vault, but it will not touch third-party config files until you confirm here.',
    ...
    <Dialog open={open} onOpenChange={(next) => { if (!next) onClose() }}>
      <DialogContent showCloseButton={false} className="sm:max-w-[520px]" data-testid="mcp-setup-dialog">
        ...
        <div className="space-y-3 text-sm leading-6 text-muted-foreground">
          <p>
            Confirming this action will write or update Grimoire&apos;s single <code className="rounded bg-muted px-1 py-0.5 text-xs">grimoire</code> MCP entry in:
          </p>
