...
vi.mock('./grimoireEditorFormattingConfig', () => ({
  getGrimoireSlashMenuItems: vi.fn(async () => []),
}))

vi.mock('./grimoireEditorFormatting', () => ({
  GrimoireFormattingToolbar: () => <div data-testid="grimoire-formatting-toolbar" />,
  GrimoireFormattingToolbarController: (props: Record<string, unknown>) => {
    state.capturedToolbarProps = props
    return <div data-testid="grimoire-formatting-toolbar-controller" />
  },
}))

vi.mock('./grimoireBlockNoteSideMenu', () => ({
  GrimoireSideMenu: () => <div data-testid="grimoire-side-menu" />,
}))
...
describe('SingleEditorView', () => {
  ...
  it('registers the seeded BlockNote test bridge, applies column widths, and cleans it up on unmount', async () => {
    ...
    expect(typeof window.__grimoireTest?.seedBlockNoteTable).toBe('function')

    await act(async () => {
      await window.__grimoireTest?.seedBlockNoteTable?.([120, null, 80])
    })
    ...
    expect(window.__grimoireTest?.seedBlockNoteTable).toBeUndefined()
  })
