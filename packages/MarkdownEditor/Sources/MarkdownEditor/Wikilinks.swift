...
public enum Frontmatter {
    ...
    public static func split(_ content: String) -> FrontmatterSplit {
        ...
        let searchStart = content.index(content.startIndex, offsetBy: 3)
        guard let endRange = content.range(of: "\n---", range: searchStart..<content.endIndex) else {
            return FrontmatterSplit(frontmatter: "", body: content)
        }

        var boundaryEnd = endRange.upperBound
        if boundaryEnd < content.endIndex, content[boundaryEnd] == "\n" {
            boundaryEnd = content.index(after: boundaryEnd)
        }

        return FrontmatterSplit(
            frontmatter: String(content[..<boundaryEnd]),
            body: String(content[boundaryEnd...])
        )
    }
}
...
public enum Wikilinks {
    ...
    public static func preprocess(_ markdown: String) -> String {
        replaceWikilinks(in: markdown) { inner in
    ...
    public static func countWords(in content: String) -> Int {
        ...
        let withoutWikilinks = withoutTitle.replacingOccurrences(
            of: #"\[\[[^\]]*\]\]"#,
            with: "",
            options: .regularExpression
        )
