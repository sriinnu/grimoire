...
async function waitForDispatchBrowserMenuCommand(page: Page): Promise<void> {
  await page.waitForFunction(
    () => typeof window.__grimoireTest?.dispatchBrowserMenuCommand === 'function',
    undefined,
    { timeout: 5_000 },
  )
}
...
async function attemptTriggerMenuCommandInPage(commandId: string): Promise<boolean> {
  const triggerMenuCommand = window.__grimoireTest?.triggerMenuCommand
...
function dispatchMenuCommandFallbackInPage(commandId: string): void {
  const dispatchBrowserMenuCommand = window.__grimoireTest?.dispatchBrowserMenuCommand
...
export async function seedBlockNoteTable(
  ...
  await page.evaluate((widths) => {
    const bridge = window.__grimoireTest?.seedBlockNoteTable
    if (typeof bridge !== 'function') {
      throw new Error('Grimoire test bridge is missing seedBlockNoteTable')
    }
...
export async function seedAutoGitSavedChange(page: Page): Promise<void> {
  await page.evaluate(async () => {
    const bridge = window.__grimoireTest?.seedAutoGitSavedChange
    if (typeof bridge !== 'function') {
      throw new Error('Grimoire test bridge is missing seedAutoGitSavedChange')
    }
    await bridge()
  })
}
...
export async function dispatchShortcutEvent(
  ...
  await page.evaluate((eventInit) => {
    const bridge = window.__grimoireTest?.dispatchShortcutEvent
    if (typeof bridge !== 'function') {
      throw new Error('Grimoire test bridge is missing dispatchShortcutEvent')
    }
    bridge(eventInit)
  }, init)
}
...
export async function triggerShortcutCommand(
  ...
  await page.evaluate((payload) => {
    const bridge = window.__grimoireTest?.triggerShortcutCommand
    if (typeof bridge !== 'function') {
      throw new Error('Grimoire test bridge is missing triggerShortcutCommand')
    }
