...
#[test]
fn test_save_note_content_creates_nested_parent_inside_active_vault() {
    let dir = tempfile::TempDir::new().unwrap();
    let note = dir.path().join("nested").join("note.md");

    save_note_content(
        note.clone(),
        "# Nested\n".to_string(),
        vault_path_arg(dir.path()),
    )
    .expect("expected save to create a missing in-vault parent folder");

    assert_eq!(std::fs::read_to_string(note).unwrap(), "# Nested\n");
}

#[test]
fn test_create_note_content_creates_nested_parent_inside_active_vault() {
    let dir = tempfile::TempDir::new().unwrap();
    let note = dir.path().join("nested").join("note.md");
    ...
    .expect("expected create to create a missing in-vault parent folder");
