...
#[cfg(desktop)]
#[tauri::command]
pub async fn init_git_repo(vault_path: VaultPathArg) -> Result<(), String> {
    let vault_path = expand_tilde(&vault_path).into_owned();
    tokio::task::spawn_blocking(move || crate::git::init_repo(&vault_path))
        .await
        .map_err(|e| format!("Task panicked: {e}"))?
}
...
#[cfg(mobile)]
#[tauri::command]
pub async fn init_git_repo(_vault_path: VaultPathArg) -> Result<(), String> {
