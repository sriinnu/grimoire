...
/// Previous default AGENTS.md content seeded by Grimoire itself. Existing vaults
/// can still contain this exact text, so Grimoire treats it as managed content
/// that is safe to refresh automatically.
const STALE_AGENTS_MD: &str = r##"# AGENTS.md — Grimoire Vault

This is a [Grimoire](https://github.com/sriinnu/grimoire) vault - a folder of markdown files with YAML frontmatter forming a personal knowledge graph.

Keep edits compatible with Grimoire's current conventions. Prefer small, human-readable changes over heavy restructuring.
...
- Frontmatter properties that start with `_` are usually Grimoire-managed state. Leave them alone unless the user explicitly asks for them to change.
...
Grimoire still understands some legacy aliases such as `Is A`, but prefer `type:` for new or edited notes.
...
/// Older Grimoire-managed AGENTS.md content from before the `type:` migration.
/// Existing vaults can still contain this exact text, so Grimoire treats it as
/// managed content that is safe to refresh automatically.
const PRE_TYPE_AGENTS_MD: &str = r##"# AGENTS.md — Grimoire Vault
...
const OUTDATED_AGENTS_MARKERS: [&str; 3] = [
    "# AGENTS.md — Grimoire Vault",
    "Legacy `title:` frontmatter is still read as a fallback",
    "Grimoire still understands legacy aliases such as `Is A`.",
];
...
/// Describes Grimoire vault mechanics only; no user-specific structure.
...
Saved views live in `views/*.yml` and are written as YAML. Grimoire scans every `.yml` file in `views/`, and the filename is the stable view id, so use kebab-case filenames such as `active-projects.yml`.
...
- Relationship filters can use wikilinks in `value`, for example `"[[grimoire]]"`.
...
pub(super) const LEGACY_AGENTS_MD: &str = r##"# AGENTS.md — Grimoire Vault
...
fn getting_started_repo_url() -> String {
    std::env::var("GRIMOIRE_GETTING_STARTED_REPO_URL")
        .or_else(|_| std::env::var("GRIMOIRE_GETTING_STARTED_REPO_URL"))
...
fn refresh_cloned_vault_config_files(vault_path: &Path) -> Result<(), String> {
    ...
    crate::git::git_commit(
        ...
        "Initialize Grimoire config files",
    )?;
    Ok(())
}
...
mod tests {
    ...
    fn write_grimoire_config_files(path: &Path) {
    ...
    #[test]
    fn test_default_getting_started_repo_url_uses_grimoire_slug() {
        assert_eq!(
            GETTING_STARTED_REPO_URL,
            "https://github.com/sriinnu/grimoire-getting-started.git"
        );
    ...
    #[test]
    fn test_canonical_getting_started_path_rejects_plain_grimoire_folder() {
        let dir = tempfile::TempDir::new().unwrap();
        let default_path = dir.path().join("Getting Started");

        write_grimoire_config_files(&default_path);
    ...
    fn test_agents_refresh_detection_accepts_legacy_json_view_guidance() {
        let stale = r#"# AGENTS.md — Grimoire Vault
...
"#;
    ...
    #[test]
    fn test_agents_template_matches_current_grimoire_vault_conventions() {
        assert!(AGENTS_MD.starts_with("---\ntype: Note\n_organized: true\n---\n"));
        assert!(AGENTS_MD.contains("# AGENTS.md — Grimoire Vault"));
