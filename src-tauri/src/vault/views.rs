...
/// Migrate views from `.grimoire/views/` to `views/` in the vault root (one-time).
pub fn migrate_views(vault_path: &Path) {
    let old_dir = vault_path.join(".grimoire").join("views");
    if !old_dir.is_dir() {
        return;
    }
...
"#;
    ...
    fn test_migrate_views_from_old_location() {
        let dir = tempfile::TempDir::new().unwrap();
        let old_dir = dir.path().join(".grimoire").join("views");
