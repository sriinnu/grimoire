...
/// Return the cache directory. Override with `GRIMOIRE_CACHE_DIR` env var (for tests).
fn cache_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("GRIMOIRE_CACHE_DIR") {
...
fn legacy_cache_path(vault: &Path) -> PathBuf {
    vault.join(".grimoire-cache.json")
}
...
mod tests {
    ...
    /// Serialize all cache tests that mutate the GRIMOIRE_CACHE_DIR env var.
    ...
    fn set_test_cache_dir(dir: &Path) {
        std::env::set_var("GRIMOIRE_CACHE_DIR", dir.to_string_lossy().as_ref());
    }
