...
Run `/grimoire-next-task` — fetches next task (To Rework first, then Open), moves to In Progress, returns full description.
...
pnpm tauri dev &
sleep 10
bash ~/.openclaw/skills/grimoire-qa/scripts/focus-app.sh grimoire
bash ~/.openclaw/skills/grimoire-qa/scripts/screenshot.sh /tmp/qa-native.png
```
...
After both phases pass, add a **completion comment** to the Todoist task before running `/grimoire-done`. The comment must include:
...
Then run `/grimoire-done <task_id>` → moves to In Review, self-dispatches next task.
...
### Code size and comments (mandatory)

- Keep every source file at **400 lines or fewer**. If a file would exceed that, split it by responsibility before committing.
- Add JSDoc to exported functions, components, hooks, types, and command handlers.
- Add comments for non-obvious control flow, platform gotchas, invariants, and failure handling. Do not comment obvious assignments or restate the code.
...
**If CodeScene gate blocks your push:** use `mcp__codescene__code_health_score` to find the worst file, refactor it, commit, push again. Do NOT stop or wait for grimoire-refactor — that is a background loop, not a substitute for fixing your own regressions.
...
Default to `demo-vault-v2/`. If you must use `~/Grimoire/` for testing:
- **Never commit or push** any test notes to the remote vault
- **Delete all test notes from disk** when done — do not leave untitled or temporary notes on the filesystem. Run `cd ~/Grimoire && git checkout -- . && git clean -fd` to restore the vault to its last committed state.
...
**When in doubt:** search `src/components/` for an existing component before building new. **Visual language:** all new UI must feel native to Grimoire — if it looks like a browser default, it's wrong.
...
```bash
bash ~/.openclaw/skills/grimoire-qa/scripts/focus-app.sh grimoire
bash ~/.openclaw/skills/grimoire-qa/scripts/screenshot.sh /tmp/out.png
bash ~/.openclaw/skills/grimoire-qa/scripts/shortcut.sh "command" "s"
```
