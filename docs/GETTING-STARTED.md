...
- `demo-vault-v2/` is the small checked-in QA fixture used for native/manual Grimoire flows. It is intentionally curated around a handful of search, relationship, project-navigation, and attachment scenarios.
...
| `src/components/SingleEditorView.tsx` | Shared BlockNote shell, Grimoire formatting controllers, and suggestion menus. |
| `src/components/editorSchema.tsx` | Custom wikilink inline content type definition. |
| `src/components/grimoireEditorFormatting.tsx` | Markdown-safe formatting toolbar surface for BlockNote. |
| `src/components/grimoireEditorFormattingConfig.ts` | Filters toolbar and slash-menu commands to markdown-roundtrippable actions. |
...
- `window.__grimoireTest.triggerShortcutCommand()` for deterministic renderer shortcut-event coverage
- `window.__grimoireTest.triggerMenuCommand()` for deterministic native menu-command coverage
