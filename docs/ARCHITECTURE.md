...
- `lib/i18nCore.ts`, `lib/i18nCommands.ts`, and `lib/i18nNoteList.ts`: startup-safe locale, command, and note-list copy. Full Settings, appearance, and portability translations stay in the lazy Settings boundary so note-first startup does not import every provider/import/export label.
...
- `utils/vaultExport.ts`: settings-triggered Markdown ZIP/static HTML export target picking, native command calls, and result feedback.
...
- `vault/exporter.rs` and `vault/html_exporter.rs`: portable Markdown ZIP and static HTML exports that refuse to write inside the active vault and share local-only exclusion policy. ZIP export writes to a same-folder temporary file before replacing the target.
