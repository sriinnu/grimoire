...
Grimoire is a personal knowledge and life management desktop app. It reads a vault of markdown files with YAML frontmatter and presents them in a four-panel UI inspired by Bear Notes.
...
When deciding where to persist a piece of data, ask: **"Would the user want this to follow them across all their Grimoire installations — other devices, future platforms (tablet, web)?"**
...
2. **Cache** — `~/.grimoire/cache/<hash>.json`, an index for fast startup. Always reconstructible from the filesystem.
...
| Cache | `scan_vault_cached()` in `vault/cache.rs` | `~/.grimoire/cache/` | Filesystem + git diff |
...
│Topics  │             │                         │            │
├────────┴─────────────┴─────────────────────────┴────────────┤
│ StatusBar: v0.4.2 │ main │ Synced 2m ago │ Vault: ~/Grimoire │
└──────────────────────────────────────────────────────────────┘
```
...
4. **MCP Integration** — Claude receives the generated MCP config file path, while Codex receives the same Grimoire MCP server via transient `-c mcp_servers.grimoire.*` config overrides
...
Each CLI agent authenticates itself outside Grimoire. Claude Code uses its existing CLI login; Codex surfaces a friendly prompt to run `codex login` when needed. Grimoire does not store model-provider API keys in app settings.
...
| `ui_open_note` | `path` | Open a note in the Grimoire UI editor |
...
- **WebSocket** — live bridge for Grimoire app integration:
...
Grimoire can register itself as an MCP server in:
...
| `register_mcp(vault_path)` | Writes Grimoire entry to Claude Code, Cursor, and generic MCP configs on explicit user request |
| `remove_mcp()` | Removes Grimoire's MCP entry from Claude Code, Cursor, and generic MCP configs |
...
3. **Appearance runtime** (`src/lib/appearance.ts`, `useAppearanceSettings`): Applies `data-theme`, `data-theme-preset`, `data-editor-font`, and the shadcn-compatible `.dark` class before React consumers render, with localStorage mirrors to avoid startup flash.

## Localization

Grimoire's app chrome uses an app-owned localization layer in `src/lib/i18n.ts` (see [ADR-0084](adr/0084-app-localization-foundation.md)). English is the canonical fallback, and Simplified Chinese (`zh-Hans`) is the first additional locale. The installation-local `ui_language` setting stores an explicit locale when the user chooses one; `null` means "follow the system language when Grimoire supports it, otherwise English." Missing translation keys fall back to English so partially translated locales do not render broken placeholders.
...
Persisted at `~/.config/com.grimoire.app/vaults.json` (reads legacy `com.tolaria.app` and `com.laputa.app` on upgrade):
```json
{
...
The starter content no longer lives in the app repo. `src-tauri/src/vault/getting_started.rs` holds the public starter repo URL (`sriinnu/grimoire-getting-started`), delegates the clone to the git backend, then normalizes Grimoire-managed root guidance and type scaffolding (`AGENTS.md`, `CLAUDE.md`, `type.md`, `note.md`) so fresh starter vaults pick up the current defaults even when the remote starter repo still carries a legacy copy or an older pre-`type:` `is_a`-era template. `AGENTS.md` stays the canonical vault guidance file; `CLAUDE.md` is a compatibility shim that imports it for Claude Code without duplicating the instructions, and Grimoire seeds it as an organized `Note` so it stays out of the way in a fresh vault. The clone helper still accepts the legacy `GRIMOIRE_GETTING_STARTED_REPO_URL` environment override so older automation can continue to redirect the starter source during the transition.

After the clone completes, Grimoire removes every configured git remote from the new starter vault. Getting Started vaults therefore open as local-only by default, and users opt into a remote later with the explicit Add Remote flow.

### Remote Clone & Auth Model

Grimoire no longer implements provider-specific OAuth or remote-repository APIs. All remote git work goes through the user's existing system git configuration.
...
- No provider tokens are stored in Grimoire settings
...
`useGitRemoteStatus` re-checks `git_remote_status` when the commit dialog opens and again right before submit. If `hasRemote` is false, Grimoire keeps the flow local-only: the status bar shows a neutral `No remote` chip, the dialog copy switches from "Commit & Push" to "Commit", and no `git_push` call is attempted.
...
| `create_getting_started_vault` | Clone the public Getting Started vault, refresh Grimoire-managed guidance/config defaults, and keep the cloned repo clean |
| `get_vault_ai_guidance_status` | Report whether `AGENTS.md` and the `CLAUDE.md` shim are managed, missing, broken, or custom |
| `restore_vault_ai_guidance` | Restore any missing/broken Grimoire-managed guidance files without overwriting custom ones |
...
| Command | Description |
|---------|-------------|
| `get_vault_settings` | Read `.grimoire/settings.json` |
...
| `remove_mcp_tools` | Remove Grimoire's MCP entry from Claude/Cursor/generic config |
...
- Deterministic QA uses two explicit proof paths from the shared manifest:
  - renderer shortcut-event proof through `window.__grimoireTest.triggerShortcutCommand()`
...
push to main
  ...
  → release job:
      ...
      → publish GitHub prerelease alpha-vYYYY.M.D-alpha.NNNN named Grimoire Alpha YYYY.M.D.N
  → pages job:
...
push stable-vYYYY.M.D tag
  ...
  → release job:
      ...
      → publish GitHub release Grimoire YYYY.M.D
  → pages job:
...
Grimoire uses the Tauri updater plugin for automatic updates:
