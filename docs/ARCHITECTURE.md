...
- `components/AiPanelIntelligenceRail.tsx`: AI-panel intelligence surfaces above the chat stream. It owns the Context Capsule preview/dialog, Crystallize loop card, and Red-Team preview dialog so `AiPanel.tsx` stays focused on controller wiring, message history, composer, and the actual Crystallize write path. The review dialog itself owns the source-safe runway for sources, Locality Firewall state, editable diff size, and local Markdown landing before apply.
...
- `utils/codeLanguageDetect.ts`: conservative display-only language detection for unlabeled fenced code blocks. Detected tags are applied to the live editor after blocks are inserted and stripped before any serialization, so they never reach markdown on disk; only a language the user picks explicitly persists.
...
- `components/NeighborhoodMap.tsx` and `utils/noteNeighborhood.ts`: the active note's one-hop neighborhood in the Second Brain panel as a two-directional map — pages linking here on one side, pages this note reaches on the other — with local-only notes visibly marked.
- `hooks/useUnlinkedMentions.ts`: Second Brain rows for prose occurrences of the active note's title or aliases that are not yet wikilinks, backed by the native mention scan, with a one-click rewrite into a Spelllink.
...
- `components/GraphModal.tsx`, `components/GraphCanvas.tsx`, `components/GraphControlPanel.tsx`, and `components/GraphRelationshipInspector.tsx`: relationship-first graph navigation map with scope controls, edge filters, type visibility toggles, zoom controls, a force-directed layout, and selected-node link/backlink inspection.
- `utils/noteGraph.ts`: graph data derived from vault entries. `buildNoteGraphCached` resolves links and degrees once per entries array so the graph modal, neighborhood, and agent-context surfaces share one structure without stale-cache risk.
...
- `vault/`: scanning, parsing, cache, rename, views, fixtures, and migration helpers. `reload_vault_soft` in `commands/vault/scan_cmds.rs` reuses the cache for incremental reloads; callers that know exactly which file changed pass vault-relative `extra_paths` so gitignored notes stay fresh without a full rescan.
- `mentions.rs` and `commands/vault/mentions_cmds.rs`: unlinked-mention scan and atomic wikilink rewrite for the Second Brain rail. Matching is whole-word and skips wikilinks, markdown links, code spans/fences, and frontmatter; local-only notes are never scanned or rewritten, though scanning for a local-only note's title in public notes is allowed.
- `code_intelligence.rs`: tree-sitter symbol and import snapshots for code files, size-capped and read-only.
...
- `vault/object_storage_s3_sync.rs`, `vault/object_storage_s3_target.rs`, `vault/object_storage_s3_plan.rs`, and `vault/object_storage_s3_io.rs`: first real S3 provider preview/apply adapter. It uses the standard local AWS credential chain, keeps credentials out of the vault, preserves the same local-only exclusions and exact preview-signature apply gate as the local-mirror prototype, and exposes only a generic redacted provider target in report-facing payloads while the transient bucket/prefix still participates in the opaque apply signature.
...
- `vault/object_storage_azure_sync.rs`, `vault/object_storage_azure_target.rs`, `vault/object_storage_azure_plan.rs`, `vault/object_storage_azure_io.rs`, and `vault/object_storage_azure_cli.rs`: Azure Blob provider preview/apply adapter using local Azure CLI login. It keeps account/container/prefix as transient scope, preserves local-only exclusions and exact preview-signature apply, exposes only a generic redacted provider target in report-facing payloads, and never stores Azure credentials in the vault.
...
- `components/GraphCanvas.tsx` renders selectable graph nodes, degree badges, zoom controls, and semantic-zoom labels.
- `hooks/useForceSimulation.ts` animates node positions with a local force simulation — interactions reheat it, it cools to zero frames at idle — and owns viewport zoom/pan state.
- `components/GraphRelationshipInspector.tsx` renders the selected node's incoming and outgoing relationship rows with edge labels and local-only markers, so connections stay legible without reverse-engineering the drawing.

Provider-bound AI context uses a stricter shape than owner-visible review UI. `lib/providerPromptPrivacy.ts` builds the provider-safe prompt draft used by the composer, dashboard `/ask`, and inline wikilink send: protected inline wikilinks are redacted before send, public wikilinks are canonicalized to stable public references, protected contradiction labels are counted instead of named, and protected note/graph omissions become policy markers. Owner review surfaces may show local held counts because they do not leave the app.
