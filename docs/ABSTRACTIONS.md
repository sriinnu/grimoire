...
Dream Forge is the private dream/journal intelligence surface. The dashboard implementation is metadata-first: it counts protected Dream and Journal notes, shows private-safe recency, and summarizes frontmatter or relationship signals such as symbols, emotional weather, and recurring people. New desktop Journal/Dream notes carry explicit agent/export/sync context gates in Markdown frontmatter, matching the mobile capture contract so every runtime can inspect why the lane stays local by default. It does not read note bodies, show dream titles, call cloud services, or export dream content.

Time Loom is the dashboard's temporal graph preview. It groups Markdown activity by local day and type, shows only coarse status buckets, and counts local-only events through the Locality Firewall without returning protected titles, paths, snippets, custom protected type labels, or body-derived content. Its summary deliberately has two scopes: full sanitized `calendarDays` for the reusable personal calendar and short recent `buckets` for the compact dashboard graph/list. Mobile capture drafts enter Time Loom as metadata-only counts using their `captured_at` chronology for unscheduled captures; scheduled mobile captures count as both calendar and mobile activity, while device class, capture source, titles, paths, and body text stay out of the temporal surface. Task and Todo due frontmatter enters as task counts and due-date placement only; task titles, project labels, paths, and raw due-key names do not leave the builder. Daily Thread uses those count-only Time Loom lanes plus Dream Forge rhythm counts to suggest one local action, such as reviewing private captures, opening the dream lane, or starting a source-safe Crystallize ask. The Crystallize action seeds `/ask` with a review-before-write memory proposal prompt, visible ask-context preview, and structured Daily Thread intent; it does not auto-write memory or expose private labels as provider context.
...
Unlinked mentions — prose occurrences of a note's title or alias that are not yet wikilinks — are discoverable from the Second Brain panel and can be rewritten into Spelllinks one at a time. Local-only notes are never scanned or rewritten by that feature.
...
The Knowledge Graph uses the same mental model visually: the user can inspect the active note's neighborhood or widen to the vault graph. The Second Brain panel renders the same one-hop neighborhood as an ambient two-directional map — pages linking here and pages this note reaches — with local-only notes visibly marked.
...
- `components/GraphCanvas.tsx` owns the SVG node surface, selected-node state styling, degree badges, zoom controls, and semantic-zoom labels.
- `hooks/useForceSimulation.ts` owns node motion and viewport zoom/pan: interactions reheat a local force simulation that cools to zero frames at idle.
- `components/GraphControlPanel.tsx` owns scope, edge-filter, and type-visibility controls.
- `components/GraphRelationshipInspector.tsx` owns the selected node's incoming/outgoing relationship rows with edge labels and local-only markers.
...
The graph modal is a relationship-first navigation map, not an agent handoff surface. Selecting a node explains its links and backlinks; local-only notes stay visibly marked, and opening one is normal local navigation, not an egress action.
