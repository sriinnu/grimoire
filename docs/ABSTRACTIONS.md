...
Key abstractions and domain models in Grimoire.

## Design Philosophy

Grimoire's abstractions follow the **convention over configuration** principle: standard field names and folder structures have well-defined meanings and trigger UI behavior automatically. This makes vaults legible both to humans and to AI agents — the more a vault follows conventions, the less custom configuration an AI needs to navigate it correctly.
...
These frontmatter field names have special meaning in Grimoire's UI:
...
Relationship fields are detected dynamically — any frontmatter field containing `[[wikilink]]` values is treated as a relationship (see [ADR-0010](adr/0010-dynamic-wikilink-relationship-detection.md)). Grimoire's own default relationship vocabulary uses snake_case on disk, but labels are humanized at render time and existing user-authored keys are left untouched.
...
- It is used by Grimoire internally for configuration, behavior, and UI preferences
...
Grimoire separates **display title** from the file identifier:
...
`SidebarSelection.kind === 'entity'` is Grimoire's Neighborhood mode for note-list browsing.
...
1. Reads cache from `~/.grimoire/cache/<vault-hash>.json` (external to vault)
...
7. On first run, migrates any legacy `.grimoire-cache.json` from inside the vault
...
- Grimoire keeps `defaultLanguage: "text"` so unlabeled code blocks do not silently become JavaScript while still supporting the packaged language aliases such as `ts` → `typescript`.
...
- Raw CodeMirror mode always shows the plain Markdown source, so imported technical notes stay editable outside Grimoire.

### Formatting Surface Policy

Defined in `src/components/grimoireEditorFormatting.tsx` and `src/components/grimoireEditorFormattingConfig.ts`:

- `SingleEditorView` disables BlockNote's default formatting toolbar, `/` menu, and side menu, then mounts Grimoire-owned controllers so the visible formatting surface matches Grimoire's markdown round-trip guarantees.
- The formatting toolbar only exposes inline controls that persist through `blocksToMarkdownLossy()` in Grimoire's save pipeline: bold, italic, strike, nesting, and link creation. Controls that BlockNote can render temporarily but Grimoire cannot faithfully persist, such as underline, color, alignment, and the block-type dropdown, are hidden instead of appearing to work and later disappearing.
- Grimoire's formatting-toolbar controller also keeps file/image actions mounted across the tiny hover gap between an image block and the floating toolbar, and while the toolbar itself is hovered, so image controls remain usable instead of collapsing mid-interaction.
- The `/` slash menu remains the supported path for markdown-safe block transformations such as headings, quotes, and list blocks. Grimoire filters out BlockNote's toggle-heading and toggle-list variants because those do not map cleanly to the markdown note model.
- The block-handle side menu keeps only actions that survive Grimoire's markdown round-trip. Delete and table-header toggles remain available; BlockNote's `Colors` submenu is removed because block colors are not part of Grimoire's supported markdown surface.
...
The app uses internal light and dark themes owned by Grimoire (see [ADR-0081](adr/0081-internal-light-dark-theme-runtime.md)). The previous vault-authored theming system remains removed; theme mode is an installation-local app preference.
...
3. **Runtime appearance bridge**: Applies `data-theme`, `data-theme-preset`, `data-editor-font`, and `.dark` for shadcn/ui, while CodeMirror and editor-specific consumers derive any non-CSS-variable values from the same semantic contract
...
- Persists vault list to `~/.config/com.grimoire.app/vaults.json` (reads legacy `com.tolaria.app` and `com.laputa.app` on upgrade)
...
Grimoire tracks managed vault-level AI guidance separately from normal note content:
- `AGENTS.md` is the canonical managed guidance file for Grimoire-aware coding agents
...
- `restore_vault_ai_guidance` repairs only Grimoire-managed files; user-authored custom `AGENTS.md` / `CLAUDE.md` files are surfaced as custom and left untouched
...
- User can create a new empty vault, open an existing folder, or clone the public Getting Started vault into a chosen parent folder; Grimoire derives the final `Getting Started` child path before cloning
- After the starter repo clone completes, Grimoire removes every remote so the new vault opens local-only by default
- Welcome state tracked in localStorage (`grimoire_welcome_dismissed`, with legacy fallback)
...
Grimoire delegates remote auth to the user's system git setup:
...
App-level settings persisted at `~/.config/com.grimoire.app/settings.json` (reads legacy `com.tolaria.app` and `com.laputa.app` on upgrade):
