...
| Live readiness audit | Verified | `pnpm test:public-readiness-audit` covers the audit model, including signed HEAD proof, clean worktree proof, CI workflow run/head/matrix proof, required macOS/Windows/Linux updater platform payload proof, starter-vault mirror drift, and release-preflight blockers. `pnpm audit:public-readiness -- --branch main` is expected to fail while public releases are missing, update feeds return `404`, and release preflight is blocked. |
