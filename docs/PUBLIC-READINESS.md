...
| Vault switching guard | Partially verified | The bottom-bar Open local folder path verifies and persists a folder before switching, rejects unavailable folders, and coalesces duplicate picker clicks while the native dialog is pending. Regressions live in `src/hooks/useVaultSwitcher.test.ts` and `src/hooks/vaultSwitcherOpenLocalAction.test.ts`. Manual native bottom-bar picker selection QA is still required before public release. |
