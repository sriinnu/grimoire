...
| Source evaluation path | Ready | [Source Evaluation Playbook](SOURCE-EVALUATION-PLAYBOOK.md) gives public source reviewers a browser mock tour, a native source tour, cleanup expectations for `demo-vault-v2/`, and explicit claim boundaries so mock UI review is not mistaken for native file-IO, packaged resource, update-feed, or installer evidence. |
...
| Live readiness audit | Verified | `pnpm test:public-readiness-audit` covers the audit model, including signed HEAD proof, clean worktree proof, CI workflow run/head/matrix/native-link/native-startup proof, required macOS/Windows/Linux updater platform payload proof, starter-vault mirror drift, and release-preflight blockers. `pnpm audit:public-readiness -- --branch main` is expected to fail while public releases are missing, update feeds return `404`, and release preflight is blocked. |
...
| macOS native launch | Locally verified | `/Applications/Grimoire.app` 0.1.431 installs, passes `codesign --verify --deep --strict`, and the native startup smoke reports `GRIMOIRE_NATIVE_STARTUP_SMOKE_READY process_entry=true` on 2026-06-15. Screenshot proof is not used for this host because `screencapture` hides normal app windows here. This is local macOS install/startup evidence, not public binary-release evidence. |
...
| Native source startup smoke | Verified | Hosted CI now runs `Native Tauri Startup Smoke` on the pinned macOS, Windows, and Linux runners after the frontend build and native link smoke. The step executes `pnpm test:native-tauri-startup`, which launches the source-mode Tauri process with `GRIMOIRE_NATIVE_STARTUP_SMOKE=1`, uses a temporary home/config directory, waits for the Rust-side `GRIMOIRE_NATIVE_STARTUP_SMOKE_READY process_entry=true` marker from the app entry point, and exits. Linux runs under Xvfb. The public-readiness audit fails if a green CI run lacks this step on any pinned OS. This is source process-entry proof, not Tauri setup, window launch, packaged installer launch proof, or manual interactive QA. |
| Browser source smoke | Verified | Hosted CI runs `Browser Smoke Chromium` and `Browser Smoke WebKit Core` on the pinned macOS runner, covering the curated source-mode smoke lane before public source readiness can pass. The public-readiness audit fails if a green CI run lacks either macOS browser smoke step, so UI/source usability proof cannot silently disappear from CI. |
