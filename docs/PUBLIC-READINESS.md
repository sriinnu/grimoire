...
| Hosted CI | Needs recheck | The latest pushed readiness-branch CI run has green `macos-15` and `ubuntu-24.04` jobs, while `windows-2025-vs2026` passed Typecheck and Build but failed in frontend tests. The current source contains additional Windows wait hardening, but that commit still needs a fresh hosted run before this row can return to verified. The latest `main` CI is still failed, so public release still needs a post-merge green `main` run. The live readiness audit reports the current run id, status, head SHA, step count, pinned runner coverage, and any GitHub check-run annotations instead of relying on a frozen failure reason. |
