...
| Hosted CI | Verified on readiness branch | Readiness-branch CI run `26834734750` completed successfully on pinned `macos-15`, `ubuntu-24.04`, and `windows-2025-vs2026` jobs for signed HEAD `8ccff8b`. This is branch source evidence, not public-release evidence: the latest `main` CI is still failed, so public release still needs a post-merge green `main` run. The live readiness audit reports the current run id, status, head SHA, step count, pinned runner coverage, and any GitHub check-run annotations instead of relying on a frozen failure reason. |
