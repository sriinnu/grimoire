...
- S3 and Azure also have ignored live write/read/delete round-trip proof tests. They only run when `GRIMOIRE_S3_LIVE_WRITE_PROOF=1` or `GRIMOIRE_AZURE_LIVE_WRITE_PROOF=1` is set, use generated proof object/blob keys under the configured prefix, compare pulled bytes, then delete the proof object. These tests prove provider primitives are reachable; Settings provider lanes still need real failure-state runs before they become provider-proven sync.
- Settings also keeps a Proof Ledger that labels ready import/export/desktop-sync lanes separately from the S3/Azure local-mirror fixture and read-only preflight state.
