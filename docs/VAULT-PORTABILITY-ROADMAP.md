...
Supported local lanes:

- Local folder: ready.
- Git remote: ready when the vault has Git enabled.
- iCloud Drive folder: folder-proof only through a local provider-managed path/readability check.
- Google Drive Desktop folder: folder-proof only through a local provider-managed path/readability check.
- Pure JSON snapshot export/import: preview-backed, with a generated local artifact loop proof.
- Local SQLite snapshot export/import: preview-backed, with a generated local artifact loop proof.

JSON/SQLite capsule import now requires a matching Locality Firewall proof before preview or apply: Markdown must remain the source of truth, absolute source paths must be redacted, and the proof's withheld count must match the withheld manifest rows. Capsule export/import writes also require the exact preview signature from the reviewed dry run, so changed vault or capsule content forces a fresh preview. General Import Autopsy writes now use the same exact-preview rule for Markdown folder, Markdown ZIP, journal, and app imports before touching disk. Those signatures include private/local-only path shape changes while withholding private file bytes, so adding hidden/runtime/private files forces a fresh review without hashing their contents. Inbound filtering follows private `grimoire-canvas` source JSON to withhold nested placed images before restore.
Settings can also run a generated JSON/SQLite capsule loop proof: native code exports a temp local capsule, previews that exact artifact for import, compares counts/signatures/locality proof, reports redacted evidence, and does not store the generated artifact path. The Settings proof card exposes those checks as a path-safe artifact checklist: export signature, import signature, matching public/local-only counts, intact Locality Firewall proof, and no stored artifact path.
...
- "Folder readable" for iCloud Drive and Google Drive Desktop means Grimoire can use provider-managed local folders and run a read-only local-folder proof for path shape, folder existence, and readability. It does not prove provider quota, offline conflict, account-auth, or cross-device merge behavior.
...
- S3 and Azure also have ignored live provider-sync proof tests. Run them with `pnpm test:object-storage-live -- --report .tmp/object-storage-live-proof.json` after setting `GRIMOIRE_S3_LIVE_WRITE_PROOF=1` or `GRIMOIRE_AZURE_LIVE_WRITE_PROOF=1` plus the required provider scope. The runner reports only set/missing env state, creates a generated proof prefix, runs Grimoire's provider preview/apply/pull path, verifies local-only journal notes stay withheld, then deletes the generated public proof object/blob. The optional report records only redacted pass/fail/missing-config evidence plus sanitized failure kind/stage categories for auth, permission, network, conflict, cleanup, gate, config, preview, apply, and pull. These tests prove the app provider contract is reachable; Settings provider lanes still need real failure-state runs before they become provider-proven sync.
- Settings also keeps a Portability Status ledger that labels ready import/export/desktop-sync lanes separately from S3/Azure proof-boundary evidence: read-only preflight, provider preview/apply contracts, and local-mirror fixtures.
- The Portability Status ledger can load a pasted redacted `grimoire-object-storage-live-proof-v1` report from the opt-in live provider proof runner, then renders only provider status, proof-gate state, set/missing config counts, and redacted failure kind/stage evidence. The sanitized report persists across Settings reopen in local app storage. It does not render or store report paths, bucket/container/account/prefix values, object keys, local absolute paths, provider stderr, or credentials.
- The Proof Ledger also turns those redacted reports into a provider failure-state coverage matrix for S3 and Azure. Each provider must record happy path, setup/config, auth/login, permission, conflict, network/retry, and cleanup states before Grimoire can call provider sync proven.
- Reviewed S3/Azure provider preview reports also surface in the Portability Status ledger as redacted direction, conflict count, local-only withheld count, preview-signature state, and "not provider-proven sync yet" evidence. The ledger never shows the provider target, local paths, object keys, or protected operation paths.
- The Settings Portability Status ledger keeps copyable no-write dry-run and live provider proof commands with the redacted report path, plus provider gate/env-name checklists without values, behind developer proof disclosure so normal Settings copy does not read like an internal test harness.
...
- Settings labels explicit S3/Azure provider apply actions as reviewed proof push/pull until real provider fixtures and failure states are recorded.
...
- Apply requires the preview signature from the exact dry run; if local or mirror files change after preview, Grimoire rejects the apply and asks for a fresh preview. Settings provider lanes also reject apply when bucket/container/account/prefix/direction changes after preview, keep apply disabled until captured preview args exist, and block conflicts instead of copying over ambiguous remote/local state.
