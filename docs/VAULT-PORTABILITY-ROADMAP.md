...
- The Portability Status ledger can load a pasted redacted `grimoire-object-storage-live-proof-v1` report from the opt-in live provider proof runner, then renders only provider status, proof-gate state, and set/missing config counts. It does not render report paths, bucket/container/account/prefix values, object keys, local absolute paths, provider stderr, or credentials.
