...
- Observed 2026-05-25: the shared HTTP MCP host can answer `http://127.0.0.1:3001/mcp` after starting `chitragupta-mcp --streamable-http --port 3001 --agent --project /path/to/grimoire` with Node 22 and restarting the daemon. Manual `chitragupta_status` succeeds only when the tool call includes `consumer: "grimoire"` and `surface: "grimoire.mcp"`; `projectPath` alone still falls through to a generic `mcp` surface and reports empty/stopped capability state.
