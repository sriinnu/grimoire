...
import {
  assertReleaseSecretChecklistCoversRequiredSecrets,
  assertReleaseSecretSetupSectionsMatchPreflight,
} from './public-readiness-release-checklist.mjs'
...
// front page and should read like a normal README, not a readiness/compliance
// artifact. The readiness contract still lives in the docs it actually governs
// (PUBLIC-READINESS.md, RELEASE-RUNBOOK.md, GETTING-STARTED.md, ...).
...
function verifyBinaryInstallTruth() {
  ...
  assertContains('src-tauri/tauri.conf.json', '"resources/starter-vault": "starter-vault/"')
  assertContains('src-tauri/tauri.conf.json', 'pnpm bundle-starter-vault')
  ...
  assertContains('docs/RELEASE-RUNBOOK.md', 'pnpm release:secrets')
  ...
  assertReleaseSecretSetupSectionsMatchPreflight(fail)
